...
$mlbTeamAbbrev = $_GET['team'];
$mlbTeamName = $_GET['teamName'];

// Set up MinorToMajorMap
$majorToMinorMap = array();

$teamDbFile = fopen("teamDb.txt", "r");
if ($teamDbFile) {
    $line = "";
    while (($line = fgets($teamDbFile)) !== false) {
        $lineArray = explode(",", $line);
        $minorAbbrev = strtoupper($lineArray[1]);
        $lgLevel = strtoupper($lineArray[3]);
        $majorAbbrev = strtoupper($lineArray[4]);
        $minorLevelAndTeamAbbrev = $lgLevel . "," . $minorAbbrev;
        // echo "Minor League: [$minorLevelAndTeamAbbrev], Major League: [$majorAbbrev]" . "<br>";
        // if ($majorToMinorMap[$majorAbbrev] === undefined) { $majorToMinorMap[$majorAbbrev] = []; }
        // Initialize if necessary:
        $majorToMinorMap[$majorAbbrev] .= ((0 == strlen($majorToMinorMap[$majorAbbrev])) ? "" : "|");
        $majorToMinorMap[$majorAbbrev] .= $minorLevelAndTeamAbbrev;
        // echo /* "$majorAbbrev : minorLevelAndTeamAbbrev is $minorLevelAndTeamAbbrev" . "<br>" */
        // print_r($majorToMinorMap[$majorAbbrev]) . "<br>";
    }
    // echo "success reading team database file! majorToMinorMap is [" . print_r($majorToMinorMap) . "]";
    fclose($teamDbFile);
} else {
    // echo "Error reading team database file!";
}

// End setting up minor to major map
function returnMlbTeamAbbrev()
{
    global $mlbTeamAbbrev;
    return $mlbTeamAbbrev;
}
...
function justReturnMap($majorTeamCode)
{
    global $majorToMinorMap;
    return $majorToMinorMap[$majorTeamCode];
}

function selectedTeamName()
{
    global $level, $mlbTeamAbbrev, $mlbTeamName;
    // $ret = "teamName is [" . $teamName . "] teamAbbrev is [" . $teamAbbrev . "], level is [" . $level . "]";
    // return $ret;
    return ($mlbTeamAbbrev == "ALL") ? "" : (" for " . $mlbTeamName);
}
...
<body ng-controller="scoreboardController"
    ...
    <h3>
        <center><?php echo($level); ?> Scoreboard<?php
    ...
    <div class="container.fluid" ng-repeat="game in scoreboards | filter:criteriaMatch('<?php echo(returnMlbTeamAbbrev()); ?>', '<?php echo(returnService()) ?>', '<?php echo(justReturnMap($mlbTeamAbbrev)); ?>')">
            ...
            <div class="row" style="background-color: #EDF6F1">
                ...
                <div class="broadcast"
                    ...
                    <i>{{game.away_name_abbrev}} TV:
                        {{game.broadcast.away.tv}}</i>
                </div>
                <div class="broadcast"
                    style="margin-left: 50px; text-align: left; float: left; font-size: 10px;"
                    ng-if="game.home_sport_code == 'mlb' && game.status.status != 'Final' && game.status.status != 'Game Over' && game.status.status != 'Postponed' && game.status.status != 'Suspended' && game.status.status != 'Cancelled'">
                    <i>{{game.home_name_abbrev}} TV:
                        {{game.broadcast.home.tv}}</i>
                </div>
            </div>
            ...
            <div class="row" style="background-color: #ffe6e6">
                <div class="now-pitching"
                    ...
                    ng-if="game.status.status == 'In Progress'">
                    <b>Pitcher:</b> {{game.pitcher.first}}
                    {{game.pitcher.last}}
            ...
            <div class="row" style="background-color: #ffffcc">
                ...
                <div class="most_recent_play"
                    style="margin-left: 50px; text-align: left; width: 540px; float: left; font-size: 12px;"
                    ng-if="game.pbp.last">
                    <b>Last play:</b> {{game.pbp.last}}
                </div>
            </div>
            <div class="row" style="background-color: #ffffcc">
            ...
            <div class="row" style="background-color: #ffffcc">
                ...
                <div class="home_runs"
                    ...
                    <b>HR:</b> <span ng-repeat="x in game.home_runs.player">
                        {{x.first}} {{x.last}} <span
                        ng-controller="abbreviationController">{{convertAbbreviation(x.team_code)}}</span>
                        ({{x.std_hr}}){{$last ? '' : ', '}}
                    </span>
                </div>
                <!-- Single Homer -->
                <div class="home_runs"
                    ...
                    <b>HR: </b> <span> {{game.home_runs.player.first}}
                        {{game.home_runs.player.last}} <span
                        ng-controller="abbreviationController">{{convertAbbreviation(game.home_runs.player.team_code)}}</span>
                        ({{game.home_runs.player.std_hr}})
                    </span>
                </div>
            </div>
